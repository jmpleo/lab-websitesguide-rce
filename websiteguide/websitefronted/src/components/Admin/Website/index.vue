    ...
    <Content :style="{padding: '0 30px'}">
      <Card dis-hover>
        <div :style="{padding:'10px 0'}">
          <Row>
            <Col span="4">
              <Button type="primary" size="default" icon="md-refresh"  @click="refresh">Обновить
              </Button>
              <Button type="error" @click="remove()">Удалить</Button>
            </Col>
          ...
          <Table :loading="loading" :columns="columns" :data="data" @on-selection-change="selectWebsite">
            <template slot-scope="{ row }" slot="action">
              <Button type="info" @click="editWebsite(row)">Редактировать</Button>
              <Button type="primary" @click="editIcon(row)">Заменить логотип</Button>
            </template>
          </Table>
          <br>
